...
title: 机器学习第三周
mathjax: true
photos: /images/a77886a6eff0f20f9d909975bb69a7ab.png
description: 逻辑回归，在这个以及接下来的几个视频中，开始介绍分类问题。在分类问题中，你要预测的变量 y是离散的值，我们将学习一种叫做逻辑回归的算法，这是目前最流行使用最广泛的一种学习算法。
tags:
 - Machine Learning
catagories:
 - Machine Learning
...
其中：
$X$ 代表特征向量
$g$ 代表逻辑函数（**logistic function**)是一个常用的逻辑函数为**S**形函数（**Sigmoid function**），公式为： {% raw %}$g\left( z \right)=\frac{1}{1+{{e}^{-z}}}${% endraw %}。
...
我们重新定义逻辑回归的代价函数为：{% raw %}$J\left( \theta  \right)=\frac{1}{m}\sum\limits_{i=1}^{m}{{Cost}\left( {h_\theta}\left( {x}^{\left( i \right)} \right),{y}^{\left( i \right)} \right)}${% endraw %}，其中
...
![Want {% raw %}${{\min }_\theta}J(\theta )${% endraw %}：](/images/171031235527.png)
...
{% raw %}${\theta_j}:={\theta_j}-\alpha \frac{1}{m}\sum\limits_{i=1}^{m}{({h_\theta}({{x}^{(i)}})-{{y}^{(i)}}){{x}_{j}}^{(i)}}${% endraw %}来同时更新所有$\theta$的值。
...
通过这样的代价函数选择出的${\theta_{3}}$和${\theta_{4}}$ 对预测结果的影响就比之前要小许多。假如我们有非常多的特征，我们并不知道其中哪些特征我们要惩罚，我们将对所有的特征进行惩罚，并且让代价函数最优化的软件来选择这些惩罚的程度。这样的结果是得到了一个较为简单的能防止过拟合问题的假设：{% raw %}$J\left( \theta  \right)=\frac{1}{2m}[\sum\limits_{i=1}^{m}{{{({h_\theta}({{x}^{(i)}})-{{y}^{(i)}})}^{2}}+\lambda \sum\limits_{j=1}^{n}{\theta_{j}^{2}}]}${% endraw %}
...
​                                                   {% raw %}${\theta_0}:={\theta_0}-a\frac{1}{m}\sum\limits_{i=1}^{m}{(({h_\theta}({{x}^{(i)}})-{{y}^{(i)}})x_{0}^{(i)}})${% endraw %} 

​                                                   {% raw %}${\theta_j}:={\theta_j}-a[\frac{1}{m}\sum\limits_{i=1}^{m}{(({h_\theta}({{x}^{(i)}})-{{y}^{(i)}})x_{j}^{\left( i \right)}}+\frac{\lambda }{m}{\theta_j}]${% endraw %} 
